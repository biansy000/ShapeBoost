...
def flip_thetas_rotmat(thetas, theta_pairs):

    thetas_flip = thetas.copy()
    # reflect horizontally
    thetas_flip[:, 0, 1:] = -1 * thetas_flip[:, 0, 1:]
    thetas_flip[:, 1:, 0] = -1 * thetas_flip[:, 1:, 0]
    # thetas_flip[:, 0] = -1 * thetas_flip[:, 0]
    # change left-right parts
    for pair in theta_pairs:
        thetas_flip[pair[0], :], thetas_flip[pair[1], :] = \
...
def get_affine_transform(center,
    ...
    ones_tmp = np.ones((3, 1))
    src_padded = np.concatenate([src, ones_tmp], axis=-1)
    dst_from_src = np.einsum('ij,bj->bi', trans, src_padded)
...
def get_affine_transform_rectangle(center,
                         scale,
                         rot,
                         output_size,
                         shift=np.array([0, 0], dtype=np.float32),
                         inv=0):
    assert isinstance(scale, np.ndarray) or isinstance(scale, list)
    ...
    rot_rad = np.pi * rot / 180
    src_dir = get_dir([src_w * -0.5, 0], rot_rad)
    dst_dir = np.array([dst_w * -0.5, 0], np.float32)

    src_dir2 = get_dir([0, src_h * -0.5], rot_rad)
    dst_dir2 = np.array([0, dst_h * -0.5], np.float32)
    ...
    src[2, :] = center + src_dir2 + scale_tmp * shift
    dst[2, :] = np.array([dst_w * 0.5, dst_h * 0.5]) + dst_dir2
...
def calculate_rectangle_width(v_hs, v_ws):
    # v_hs: vector of height, Nx3, v_ws: vector of weight, Nx3
    cross_norm = np.linalg.norm(np.cross(v_hs, v_ws), axis=-1)
    h_norm = np.linalg.norm(v_hs, axis=-1)

    return cross_norm / (h_norm + 1e-8)


def get_composed_trans(trans1, trans2, inv=False):
    # trans1, trans2: 2x3
    pad_array = np.zeros((1, 3))
    pad_array[0, -1] = 1.0
    trans1_3x3 = np.concatenate([trans1, pad_array.copy()], axis=0)
    trans2_3x3 = np.concatenate([trans2, pad_array.copy()], axis=0)

    trans_new_3x3 = np.matmul(trans2_3x3, trans1_3x3)
    # print(trans_new_3x3)
    trans_new = trans_new_3x3[:2, :]
    
    if not inv:
        return trans_new
    else:
        inv_matrix = np.linalg.inv(trans_new_3x3)
        # print(inv_matrix)
        return inv_matrix[:2, :]
...
def get_one_box(det_output, thrd=0.9):
    ...
    for i in range(det_output['boxes'].shape[0]):
        ...
        if float(score) < thrd or label != 1:
            continue
