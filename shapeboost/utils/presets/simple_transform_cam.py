...
from ..transforms import (addDPG, affine_transform, flip_joints_3d,
                          ...
                          get_composed_trans)
from hybrik.beta_decompose.beta_process2 import part_names
from hybrik.beta_decompose.beta_process_finer import part_names_finer
...
class SimpleTransformCam(object):
    ...
    def __call__(self, src, label):
        ...
        assert not self.change_widths
        if self.change_widths and self._train and random.random() < 0.5:
            width_changed = True
            scale_diff = 1.3 + (np.random.rand(2) - 0.5) # 0.8 ~ 1.8
        ...
        # cv2.imwrite(f'exp/visualize/hybrik_coco/{bbox[0]}.png', img)
        ...
        output = {
            ...
            'trans_fwd': torch.from_numpy(trans).float(),
            ...
            'part_widths': torch.zeros(len(part_names)).float(),
            'width_ratio': torch.ones(len(part_names)).float()*0.5,
