...
from shapeboost.beta_decompose.beta_process2 import vertice2capsule_fast, part_names, spine_joints, used_part_seg
...
class SimpleTransform3DSMPLCam(object):
    ...
    def __init__(self, dataset, scale_factor, color_factor, occlusion, add_dpg,
        ...
        self.dataset_name = dataset.CLASSES
    ...
    def _call_2d(self, src, label):
        ...
        if self._train and self._occlusion and random.random() < 0.5:
            while True:
                area_min = 0.0
                area_max = 0.5
    ...
    def _call_3d(self, src, label):
        ...
        # if joints_vis_29.sum() < 29 * 3:
        #     smpl_out = self.smpl_layer(
        #         pose_axis_angle=torch.from_numpy(theta).unsqueeze(0).float(),
        ...
        #     smpl_joints = smpl_out.joints.numpy()[0]

            # joint_cam_29[24:] = smpl_joints[24:]
            # if np.absolute(joint_cam_29[:24] - smpl_joints[:24] ).max() < 1e-4:
            #     joints_vis_29[:] = 1
        ...
        if self.use_2dv:
            tmp_uvd_24_weight = joints_vis_29.reshape(-1, 3)[:24]
            tmp_uv_29 = joint_img_29.reshape(-1, 3).copy()[:24] / np.array([imgwidth, imgwidth, 1000.0]) - np.array([0.5, 0.5*imght/imgwidth, 0])
            tmp_xyz = joint_cam_29.reshape(-1, 3).copy()[:24] / 2.2
            tmp_xyz = tmp_xyz - tmp_xyz[0]
            # print(self.dataset_name, tmp_uv_29, joint_cam_29.reshape(-1, 3).copy()[:24])
            if self.dataset_name[0] == 'h36m':
                tmp_xyz = tmp_xyz / 1000.0
            
            # print(self.dataset_name, tmp_uv_29, tmp_xyz)

            cam_scale_rawimg, cam_trans_rawimg, _, _, _ = self.calc_cam_scale_trans2(
                                                            tmp_xyz, 
                                                            tmp_uv_29, 
        ...
        # if self.change_widths and self._train:
            ...
            # scale_diff = np.array([1.3, 1.4]) + (np.random.rand(2) - 0.5) * 1.2 # 0.8 ~ 2.0
            scale_aspect = (random.random()*0.6 + 0.4) # 0.25 - 1.0
            scale_ratio = np.array([0.95*np.sqrt(scale_aspect), 0.95/np.sqrt(scale_aspect)])
            if random.random() > 0.67:
        ...
        # with torch.no_grad():
        #     transl_glob = torch.zeros(1, 3)
        #     transl_glob[:, :2] = torch.from_numpy(cam_trans).float()
        #     transl_glob[:, 2] = 1000 / 256.0 / cam_scale

        #     beta_torch = torch.from_numpy(beta).reshape(1, 10).float().clone()
        #     pose_torch = torch.from_numpy(theta).reshape(1, 24, 3).float().clone()
        ...
        #     width_ratio_from2d = self.get_width_ratio_from2d(
        #         beta_torch, 
        #         transl_glob, 
        #         transforms
        #     ).numpy()[0]

        #     print(width_ratio_from2d.shape, width_ratio_2d.shape)
        ...
        if self.use_2dv and 'is_valid' in label:
            ...
            v_abs_d = pred_vertices[:, [2]] + 5000 / (256.0 * cam_scale_rawimg)
            pred_2dv = (pred_vertices[:, :2] + cam_trans_rawimg.reshape(1, 2)) / v_abs_d / (256.0 / 5000.0)
            ...
            # print(self.dataset_name, cam_scale_rawimg, cam_trans_rawimg)
    ...
    def calc_cam_scale_trans2(self, xyz_29, uvd_29, uvd_weight, f=1000.0):
