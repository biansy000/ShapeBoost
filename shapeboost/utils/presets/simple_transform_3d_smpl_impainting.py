...
from shapeboost.utils.get_hm import get_heatmap, alpha_matting


class SimpleTransform3DSMPLImpaint(object):
    ...
    def _call_3d(self, src, label):
        mask_paths = label['mask_img_paths']
        mask_img_person = cv2.imread(mask_paths[0])
        mask_img_all = cv2.imread(mask_paths[1])

        mask_height, mask_width = mask_img_person.shape[:2]
        mask_img_all_person = mask_img_all.reshape(-1, 3)[(mask_img_person.reshape(-1, 3) > 0).any(axis=-1), :]
        mask_img_all_person_val = np.unique(mask_img_all_person.reshape(-1, 3), axis=0)

        smpl_vis_weight = 1

        human_mask = np.zeros_like(mask_img_person)
        filled_ratio = 0
        if len(mask_img_all_person_val) == 0:
            smpl_vis_weight = 0
        else:
            mask_person_val = np.unique(mask_img_person.reshape(-1, 3), axis=0)
            mask_person_val = mask_person_val[(mask_person_val >= 1).any(axis=-1), :].mean(axis=0).reshape(3)
            tmp_flag = (mask_img_all_person_val[0] == np.array([0, 0, 255])).all()
            
            if len(mask_img_all_person_val) == 1 and tmp_flag:
                smpl_vis_weight = 0
            else:
                diff = np.absolute(mask_img_all_person_val*1.0 - mask_person_val*1.0).sum(axis=-1)
                diff_small = diff < 5
                choosed_vals = mask_img_all_person_val[diff_small, :].reshape(-1, 3)
                if diff_small.sum() == 0:
                    min_diff = np.argmin(diff)
                    choosed_vals = mask_img_all_person_val[min_diff, :].reshape(-1, 3)
                
                for choosed_val in choosed_vals:
                    human_mask += (mask_img_all == choosed_val).all(axis=-1, keepdims=True)
                
                human_mask_filled = (human_mask.sum(axis=-1) > 0.5)
                mask_img_person_filled = (mask_img_person.sum(axis=-1) > 0.5)

                check_valid = (human_mask_filled) & (~mask_img_person_filled)
                if check_valid.sum() > 20 and len(choosed_vals) == 1 and np.absolute(choosed_vals[0]*1.0 - mask_person_val*1.0).sum() > 20:
                    smpl_vis_weight = 0

                human_mask = (human_mask_filled & mask_img_person_filled) * 1.0

                filled_ratio = human_mask_filled.sum() / (mask_img_person_filled.sum() + 1e-5)
        
        if human_mask.shape[-1] == 3:
            human_mask = human_mask.sum(axis=-1)

        joint_img_17 = label['joint_img_17'].copy().reshape(17, 3)
        kpt_cnt = 0
        for item in joint_img_17:
            u = int(item[0] + 0.5)
            v = int(item[1] + 0.5)
            if u >= 0 and u < human_mask.shape[1] and v >= 0 and v < human_mask.shape[0] and human_mask[v, u] > 0.5:
                kpt_cnt += 1
        ...
        idx = label['img_id']
        human_mask = (human_mask > 0.5) * 1.0
        bbox = list(label['bbox'])

        cutout, alpha = alpha_matting(src, human_mask)

        has_got_hm = False
        if filled_ratio > 0.8:
            has_got_hm = True
            heatmap, pos = get_heatmap(src, human_mask, bbox)
        else:
            heatmap, pos = None, None

        # # visualize
        # pa_path = 'exp/visualize/inpaint_try'
        # img_name = label['img_path'].split('/')[-1]
        # saved_path = os.path.join(pa_path, img_name.split('.')[0] + f'_{idx}_hm.png')
        # human_masked_img = src.copy().reshape(-1, 3)
        # human_masked_img[human_mask.reshape(-1) < 0.5, :] = 0
        # human_masked_img = human_masked_img.reshape(src.shape)

        # saved_path_masked = os.path.join(pa_path, img_name.split('.')[0] + f'_{idx}_masked.png')
        # saved_path_raw = os.path.join(pa_path, img_name.split('.')[0] + f'_{idx}_raw.png')

        # cv2.imwrite(saved_path, heatmap)
        # cv2.imwrite(saved_path_raw, cv2.cvtColor(src, cv2.COLOR_BGR2RGB))
        # cv2.imwrite(saved_path_masked, cv2.cvtColor(human_masked_img, cv2.COLOR_BGR2RGB))
        ...
        new_pos = False
        if filled_ratio > 0.8 and pos is not None:
            center_img = np.array([pos[1], pos[0]])
            new_pos = True
        else:
            center_img = center
        ...
        # if True:
            ...
            scale_human = scale * np.clip(np.random.randn() * sf + 1, 1 - sf, 1 + sf) * scale_ratio
            img_scale = min(scale_ratio.mean() * 1.1, scale_ratio.max())
            if filled_ratio <= 0.8:
                scale_img = scale * np.clip(np.random.randn() * sf + 1, 1 - sf, 1 + sf) * img_scale
            else:
                scale_img = scale_human.mean()
                scale_img = np.array([scale_img, scale_img])
        else:
            sf = self._scale_factor
            scale_human = scale * np.clip(np.random.randn() * sf + 1, 1 - sf, 1 + sf)
            scale_img = scale_human * np.clip(np.random.randn()*0.5 * sf + 1, 1 - 0.5*sf, 1 + 0.5*sf) * 1.1
        ...
        if random.random() > 0.75 and self._train:
            ...
            center_img[0] = imgwidth - center_img[0] - 1
        ...
        part_widths, width_ratio_2d, width_ratio, part_spines = self.width_process(
            beta.copy(), joints_29_uvd[:, :2, 0].copy(), joints_cam_29_xyz.reshape(-1, 3).copy(), r+r2, scale=scale_human)
        ...
        inp_h, inp_w = input_size
        trans_img = get_affine_transform_rectangle(center_img, scale_img, r, [inp_w, inp_h])
        trans_human = get_affine_transform_rectangle(center, scale_human, r+r2, [inp_w, inp_h])
        trans_inv = get_affine_transform_rectangle(center, scale_human, r+r2, [inp_w, inp_h], inv=True)
        ...
        img = cv2.warpAffine(src, trans_img, (int(inp_w), int(inp_h)), flags=cv2.INTER_LINEAR)
        img_human = cv2.warpAffine(src, trans_human, (int(inp_w), int(inp_h)), flags=cv2.INTER_LINEAR)

        # trans_mask = label['mask_trans_inv']
        # mask_img = cv2.warpAffine(mask_img, trans_mask, (imgwidth, imght), flags=cv2.INTER_LINEAR)
        if smpl_vis_weight:
            mask_img = (human_mask.reshape(src.shape[0], src.shape[1], 1) > 1e-5).astype(np.uint8) * 255
            if flipped:
                mask_img = mask_img[:, ::-1, :]
                
            mask_img_human = cv2.warpAffine(mask_img, trans_human, (256, 256), flags=cv2.INTER_LINEAR).reshape(256, 256, 1)
            mask_img_img = cv2.warpAffine(mask_img, trans_img, (256, 256), flags=cv2.INTER_LINEAR).reshape(256, 256, 1)

            mask_img_human = (mask_img_human[:, :, [0]] > 1e-5) * 1.0
            mask_img_img = (mask_img_img[:, :, [0]] > 1e-5) * 1.0

            impainted_img = cv2.inpaint(img, (mask_img_img*255).astype(np.uint8), 5, cv2.INPAINT_NS)

            img = impainted_img.reshape(-1, 3) * (1 - mask_img_human.reshape(-1, 1)) + \
                    img_human.reshape(-1, 3) * mask_img_human.reshape(-1, 1)
        ...
        for i in range(17):
            if joints_17_uvd[i, 0, 1] > 0.0:
                joints_17_uvd[i, 0:2, 0] = affine_transform(joints_17_uvd[i, 0:2, 0], trans_human)

        for i in range(29):
            if joints_29_uvd[i, 0, 1] > 0.0:
                joints_29_uvd[i, 0:2, 0] = affine_transform(joints_29_uvd[i, 0:2, 0], trans_human)
        ...
        if self.use_2dv and 'is_valid' in label:
            ...
            pred_2dv = np.einsum('ij,bj->bi', trans_human, pred_2dv_homo)
        ...
        output = {
            ...
            'target_beta': torch.from_numpy(beta).float(),
            'target_smpl_weight': target_smpl_weight * smpl_vis_weight,
            ...
            'width_changed_ratio': scale[1] / scale[0],
            # 'img_path': label['img_path'],
            # 'center_img': torch.tensor([center_img[0], center_img[1], new_pos]).float()
            # 'smpl_vis_weight': smpl_vis_weight
