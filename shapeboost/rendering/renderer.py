...
class TexturedIUVRenderer(nn.Module):
    ...
    def forward(self, vertices, textures=None, cam_t=None, lights_rgb_settings=None,
        ...
        # print('zbuffers', zbuffers.max())

        return output
    

    def forward_variable_batchsize(self, vertices, textures=None, cam_t=None, lights_rgb_settings=None,
                verts_features=None, img_center=None):
        ...
        textures_iuv = TexturesVertex(verts_features=self.verts_iuv[:batch_size])
        meshes_iuv = Meshes(verts=vertices, faces=self.faces_densepose[:batch_size], textures=textures_iuv)
        if self.render_rgb:
            if verts_features is not None:
                verts_features = verts_features[:, self.verts_map, :]  # From SMPL verts indexing (0 to 6889) to DP verts indexing (0 to 7828), verts shape is (B, 7829, 3)
                textures_rgb = TexturesVertex(verts_features=verts_features)
            else:
                textures_rgb = TexturesUV(maps=textures, faces_uvs=self.faces_densepose[:batch_size], verts_uvs=self.verts_uv_offset[:batch_size])
            
            # print(vertices.shape, textures_rgb, self.faces_densepose[:batch_size].shape)
            meshes_rgb = Meshes(verts=vertices, faces=self.faces_densepose[:batch_size], textures=textures_rgb)
