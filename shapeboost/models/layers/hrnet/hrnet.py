...
class PoseHighResolutionNet(nn.Module):
    ...
    def __init__(self, cfg, **kwargs):
        ...
        self.inp_size = kwargs.get('inp_size', 3)
        # stem net
        self.conv1 = nn.Conv2d(self.inp_size, 64, kernel_size=3, stride=2, padding=1,
                               bias=False)
        ...
        if self.generate_hm:
            ...
            self.extra_hm = cfg['MODEL']['extra_hm'] if 'extra_hm' in cfg['MODEL'] else -1
            if 'extra_hm' in cfg['MODEL'] and cfg['MODEL']['extra_hm'] > 0:
                self.final_layer2 = nn.Conv2d(
                    in_channels=pre_stage_channels[0],
                    out_channels=cfg['MODEL']['extra_hm'],
                    kernel_size=extra['FINAL_CONV_KERNEL'],
                    stride=1,
                    padding=1 if extra['FINAL_CONV_KERNEL'] == 3 else 0
                )
    ...
    def forward(self, x):
        ...
        if self.generate_hm:
            ...
            if self.extra_hm > 0:
                out_heatmap2 = self.final_layer2(y_list[0])
                out_heatmap = torch.cat([out_heatmap2, out_heatmap], dim=1)
...
def get_hrnet(type_name, num_joints, depth_dim, is_train, pretrain, **kwargs):
    cfg = load_hrnet_cfg(f'./shapeboost/models/layers/hrnet/w{type_name}.yaml')
    cfg['MODEL']['NUM_JOINTS'] = num_joints
    cfg['MODEL']['DEPTH_DIM'] = depth_dim
    if 'extra_hm' in kwargs:
        cfg['MODEL']['extra_hm'] = kwargs['extra_hm']
    else:
        cfg['MODEL']['extra_hm'] = 0
