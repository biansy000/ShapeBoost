...
class PW3D(data.Dataset):
    ...
    def __init__(self,
                 ...
                 skip_empty=True,
                 dpg=False,
                 lazy_import=False, return_img_path=False,
                 change_widths=True):
        self._cfg = cfg
        ...
        self.transformation = SimpleTransform3DSMPLCam(
            ...
            loss_type=self._loss_type, scale_mult=1.25, change_widths=change_widths, 
    ...
    def __getitem__(self, idx):
        ...
        # assert 'twist_phi' not in self.db.keys()
    ...
    def evaluate_xyz_17_sampling(self, preds, result_dir, num_hypo=1):
        ...
        error_pa = np.zeros((sample_num, num_hypo, len(self.EVAL_JOINTS)))  # joint error
        ...
        for n in range(sample_num):
            ...
            bbox = self.db['bbox'][n]
            gt_3d_root = self.db['root_cam'][n]
            gt_3d_kpt_all = self.db['joint_relative_17'][n]
            ...
            for ns in range(num_hypo):
                ...
                error_pa[n, ns] = np.sqrt(np.sum((pred_3d_kpt_pa - gt_3d_kpt)**2, 1))
        ...
        error_pa = np.mean(error_pa, axis=2)
        error_pa = np.min(error_pa, axis=1)
        tot_err_pa = np.mean(error_pa) * 1000
