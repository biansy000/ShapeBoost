...
class H36mSMPL(data.Dataset):
    ...
    def __init__(self,
                 ...
                 skip_empty=True,
                 dpg=False,
                 lazy_import=False, change_widths=True):
        self._cfg = cfg
        ...
        preset_trans_type = cfg.MODEL.EXTRA.PRESET
        if cfg.MODEL.EXTRA.get('AMBIGUOUS', False):
            preset_trans_type += '_amb'
        
        self._use_finer_part = cfg.MODEL.EXTRA.get('USE_FINER_PART', False)
        self._use_2dv = cfg.MODEL.EXTRA.get('USE_2DV', False)
        print('self._use_2dv', self._use_2dv)
        ...
        self.transformation = SimpleTransform3DSMPLCam(
            ...
            loss_type=self._loss_type, scale_mult=1, change_widths=change_widths, 
    ...
    def __getitem__(self, idx):
        ...
        target = self.transformation(src, label)
    ...
    def evaluate_xyz_17_sampling(self, preds, result_dir, num_hypo=1):
        print('Evaluation start (sampling)...')
        assert len(self.db['img_id']) == len(preds)
        sample_num = len(self.db['img_id'])

        pred_save = []
        error = np.zeros((sample_num, num_hypo, len(self.EVAL_JOINTS)))  # joint error
        error_align = np.zeros((sample_num, num_hypo, len(self.EVAL_JOINTS)))  # joint error
        error_x = np.zeros((sample_num, num_hypo, len(self.EVAL_JOINTS)))  # joint error
        error_y = np.zeros((sample_num, num_hypo, len(self.EVAL_JOINTS)))  # joint error
        error_z = np.zeros((sample_num, num_hypo, len(self.EVAL_JOINTS)))  # joint error
        # error for each sequence
        error_action = [[] for _ in range(len(self.action_name))]
        for n in range(sample_num):
            image_id = self.db['img_id'][n]
            bbox = self.db['bbox'][n]
            gt_3d_root = self.db['root_cam'][n].copy()
            gt_3d_kpt_all = self.db['joint_relative_17'][n].copy()

            # gt_vis = self.db['joint_vis'][n]

            # restore coordinates to original space
            pred_3d_kpt_all = preds[image_id]['xyz_17'].copy() * self.bbox_3d_shape[2]

            for ns in range(num_hypo):
                pred_3d_kpt = pred_3d_kpt_all[ns]

                # root joint alignment
                pred_3d_kpt = pred_3d_kpt - pred_3d_kpt[self.root_idx_17]
                gt_3d_kpt = gt_3d_kpt_all - gt_3d_kpt_all[self.root_idx_17]
                ...
                # error calculate
                error[n, ns] = np.sqrt(np.sum((pred_3d_kpt - gt_3d_kpt)**2, 1))
                error_align[n, ns] = np.sqrt(np.sum((pred_3d_kpt_align - gt_3d_kpt)**2, 1))
                error_x[n, ns] = np.abs(pred_3d_kpt[:, 0] - gt_3d_kpt[:, 0])
                error_y[n, ns] = np.abs(pred_3d_kpt[:, 1] - gt_3d_kpt[:, 1])
                error_z[n, ns] = np.abs(pred_3d_kpt[:, 2] - gt_3d_kpt[:, 2])
                img_name = self.db['img_path'][n]
                action_idx = int(img_name[img_name.find(
                    'act') + 4:img_name.find('act') + 6]) - 2
                error_action[action_idx].append(error[n, ns].copy())
        ...
        error = np.min(error, axis=1)
        tot_err = np.mean(error)

        error_align = np.mean(error_align, axis=2)
        error_align = np.min(error_align, axis=1)
        tot_err_align = np.mean(error_align)

        error_x = np.mean(error_x, axis=2)
        error_x = np.min(error_x, axis=1)
        tot_err_x = np.mean(error_x)

        error_y = np.mean(error_y, axis=2)
        error_y = np.min(error_y, axis=1)
        tot_err_y = np.mean(error_y)

        error_z = np.mean(error_z, axis=2)
        error_z = np.min(error_z, axis=1)
        tot_err_z = np.mean(error_z)
        metric = 'PA MPJPE' if self.protocol == 1 else 'MPJPE'
